# $Id: Visterm.pm,v 0.01 2003/01/08 05:30:09 lunartear Exp $
# Copyrights and documentation are after __END__.
package Curses::Visterm;
...
sub CUR_WIN    () { 2 } # holds the current window id
sub ERRLEVEL   () { 3 } # Visterm's Error Level boolean 
  ...
  if (defined $new_window_id) {
    ...
    if (!$self->[WINDOW]->{$new_window_id}) {
      $self->[WINDOW]->{$new_window_id} =
        ...
        _set_color($self, $new_window_id, %{$params{Palette}});
      ...
      if ($winref->{Use_Title}) {
        ...
        $title->bkgd($winref->{Palette}->{st_frames}->[PAL_PAIR]); # <- FIXME icky hardcoded bg
        $title->erase();
        #$title->noutrefresh();
...
if (DEBUG) { print ERRS $status, " <-status in create_window\n"; }
        $status->bkgd($winref->{Palette}->{st_frames}->[PAL_PAIR]); # FIXME more hardcoded crap
        $status->erase();
        $status->noutrefresh();
      ...
      # I think I'll keep ncolor hardcoded  or maybe not...
      $screen->bkgd($winref->{Palette}->{ncolor}->[PAL_PAIR]);
      $screen->erase();
      $screen->noutrefresh();
...
### my %palette = $vt->get_palette($window_id); # entire palette.
### my $color_desc = $vt->get_palette($window_id, $colorname); # color desc.

sub get_palette {  
  my $self = shift;
  if ($#_ == 0) {
    my $window_id = @_;
    return %{$self->[WINDOW]->{$window_id}->{Palette}};
  }
  elsif ($#_ == 1) { 
    my ($window_id, $query) = @_;
    return $self->[WINDOW]->{$window_id}->{Palette}->{$query}->[PAL_DESC];
  }
...
sub set_palette {
  ...
  if ($validity) {
    ...
    my %params = @_;
    _set_color($self, $window_id, %params);
  }
  else {
...
sub _refresh_buffer {
  ...
  while ($screen_y < $winref->{Screen_Height}) {
    ...
    while (length $line) {
      ...
      if ($line =~ s/^ \0 \( ([^\)]+) \) //x) {
        $screen->attrset($winref->{Palette}->{$1}->[PAL_PAIR]); 
        $screen->noutrefresh();
      }
      if ($line =~ s/^([^\0]+)//x) {
    ...
    $screen->attrset($winref->{Palette}->{ncolor}->[PAL_PAIR]); 
    $screen->noutrefresh();
    $screen->clrtoeol();
    $screen->noutrefresh();
  }
...
sub _set_color {
  ...
  for my $color_name (keys %params) {
    ...
    if (exists $winref->{Palette}->{$color_name}) {
      my $old_color_number = $winref->{Palette}->{$color_name}->[PAL_NUMBER];
      init_pair($old_color_number, $foreground, $background);
      $winref->{Palette}->{$color_name}->[PAL_PAIR] =
        COLOR_PAIR($old_color_number) | $attributes;
    }
    else {
      my $new_color_number = ++$winref->{Color_Sequence};
      init_pair($new_color_number, $foreground, $background);
      $winref->{Palette}->{$color_name} =
...
sub _refresh_title {
  ...
  $title->attrset($winref->{Palette}->{st_values}->[PAL_PAIR]); #FIXME icky hardcode
  $title->noutrefresh();
...
sub set_status_format {
  ...
  my $self = shift;
  my $window_id = shift;
#  croak "set_status_format requires an even number of parameters" if @_ & 1;
  my %status_formats = @_;
...
if (DEBUG) { print ERRS $status, "<-status ref\n"; }
    ...
    while (defined $value and length $value) {
      ...
      if ($value =~ s/^\0\(([^\)]+)\)//) {
        ...
        $status->attrset($winref->{Palette}->{$1}->[PAL_PAIR]);
        $status->noutrefresh();
      }
      if ($value =~ s/^([^\0]+)//) {
        if (DEBUG) { print ERRS "value matched", '^([^\0]+)', "\n"; }
  ...
  $status->attrset($winref->{Palette}->{st_frames}->[PAL_PAIR]); #FIXME hard coded
  $status->noutrefresh();
  $status->clrtoeol();
  $status->noutrefresh();
...
Curses::Visterm - split-terminal user interface
...
=head1 SYNOPSIS
  ...
  my $vt = Visterm->new( Alias => "interface",
                         Errlevel => 0 );
...
Create and initialize a new instance of Visterm.
...
Errlevel sets Visterm's error level.
...
Set a window's color palette

  Palette => { st_frames => "bright cyan on blue",
...
Don't use Visterm's Titlebar.

  Use_Title => 0

Don't use Visterm's StatusBar.
...
Return a windows palette or a specific colorname's description
  ...
  my %palette = $vt->get_palette($window_id);

  my $color_desc = $vt->get_palette($window_id, $colorname);

=item set_palette

  $vt->set_palette( $window_id,
                    color_name => "color on color" );
...
Visterm is Copyright 2002 Charles Ayres. All rights reserved.
Visterm is free software; you may redistribute it and/or modify
it under the same terms as Perl itself.

Questions and Comments can be sent to Visterm@ambientheory.com
